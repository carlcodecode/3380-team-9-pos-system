...
const getByIdResult = await makeRequest(`/api/meal-categories/${mealCategoryId}`, 'GET', null, staffToken);
...
const updateResult = await makeRequest(`/api/meal-categories/${mealCategoryId}`, 'PUT', updateData, staffToken);
...
if (customerLogin.data.token) {
	...
	const customerAccessResult = await makeRequest('/api/meal-categories', 'GET', null, customerToken);
...
const invalidCreateResult = await makeRequest('/api/meal-categories', 'POST', invalidCreateData, staffToken);
...
const invalidUpdateResult = await makeRequest(`/api/meal-categories/${mealCategoryId}`, 'PUT', invalidUpdateData, staffToken);
...
console.log(`14. Testing DELETE /api/meal-categories/${mealCategoryId} (delete first meal category)...`);
const deleteResult = await makeRequest(`/api/meal-categories/${mealCategoryId}`, 'DELETE', null, staffToken);
...
if (mealCategoryId2) {
	console.log(`15. Testing DELETE /api/meal-categories/${mealCategoryId2} (delete second meal category)...`);
	const deleteResult2 = await makeRequest(`/api/meal-categories/${mealCategoryId2}`, 'DELETE', null, staffToken);
